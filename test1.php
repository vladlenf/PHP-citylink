...
$a = searchWorker('Древлянка');
echo($a);
function searchWorker($area)
{
	global $areas;
	global $workers;
	global $nearby;

	foreach ($workers as $worker){ #Проверка сотрудника в введенном районе
	...
	foreach ($areas as $areaKey => $areaValue){ #Нахождение соседних районов
		if($areaValue == $area){
			$nearbyArray = $nearby[$areaKey];
		}
	}

	foreach ($nearbyArray as $value){ #Нахождение сотрудника из ближайших районов
		foreach ($workers as $worker){
			if($areas[$value] == $worker['area_name']){
				return $worker['login'];
			}
	}
